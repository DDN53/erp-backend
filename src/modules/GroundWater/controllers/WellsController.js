const Wells = require("../utils/Wells_id");
    ...
    try {
      ...
      if (!data.WELLNO) {
        return res.status(400).json({ 
          error: "WELLNO is Missing!"
        });
      }
      const existingWell = await db.query(
        'SELECT * FROM [dbo].[GWM_WellId] WHERE wellNo = :wellNo',
        { replacements: { wellNo: data.WELLNO } }
      );

      if (existingWell[0].length > 0) {
        return res.status(400).json({ error: "Well with the same WELLNO already exists!" });
      }

      const newWell = await createWell({
        ...data,
        RSCLocation: data.RSCLocation,
        ScaleGeologyMap: data.ScaleGeologyMap,
        WellType: data.WellType,
       WellCondition: data.WellCondition
      });
      res.status(201).json({ message: "Well added successfully" });
    } catch (err) {
      console.error("Error adding well:", err);
      res.status(500).json({ 
        error: "Failed to add well to the database",
